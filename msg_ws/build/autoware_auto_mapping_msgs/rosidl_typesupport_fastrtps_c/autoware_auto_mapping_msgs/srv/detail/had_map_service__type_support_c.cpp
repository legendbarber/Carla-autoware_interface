  ...
  size_t current_alignment)
{
  ...
  if (is_plain) {
    ...
    using DataType = autoware_auto_mapping_msgs__srv__HADMapService_Request;
    is_plain =
      (
      offsetof(DataType, geom_lower_bound) +
      last_member_size
      ) == ret_val;
  }
  ...
  size_t current_alignment)
{
  ...
  if (is_plain) {
    ...
    // We still need to check that the in-memory alignment
    // is the same as the CDR mandated alignment.
    using DataType = autoware_auto_mapping_msgs__srv__HADMapService_Response;
    is_plain =
      (
